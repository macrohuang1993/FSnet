classdef Camera
    properties       
        arg = struct('nx', [], 'dx', [], 'ny', [], 'dy', [], ...            % Aperture plane
                     'nu', [], 'du', [], 'nv', [], 'dv', [], ...            % Sensor plane
                     'focalLen', [], 'apeSize', [], 'apeMask', [], ...      % Camera settings
        ...
        function obj = Camera(varargin)                                      
            if nargin >= 1
                ...
                if isempty(obj.arg.refDis)
                    ...
                    % for complext scene refDis is not empty and is
                    % calculated based on the focus distance of the light
                    % field data. For not complex scene, the refDis is
                    % set to max of fsdis.
                end
