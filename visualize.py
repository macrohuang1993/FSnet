...
def show(img):
    ...
    plt.show()
def show_FS(FS,**kwargs):
    """
    one sample of FS:C,nF,H,W or a batch of FS: B,C,nF,H,W
    """
    shape = FS.shape
    if len(shape) == 4: 
        show(make_grid(FS.permute([1,0,2,3]),padding = 30,**kwargs))
    elif len(shape) == 5:
        B,C,nF,H,W = shape
        show(make_grid(FS.permute([0,2,1,3,4]).reshape(B*nF,C,H,W),nrow = nF, padding = 30,**kwargs))
def show_SAI(LF,vu_pair_list,**kwargs):
    """
    ...
    if len(shape) == 5:
        ...
        show(make_grid(SAI_list,padding = 30),**kwargs)
    elif len(shape) == 6:   
        B,C,nv,nu,H,W = shape
...
def show_EPI_xu(LF,yv_pair_list,**kwargs):
    """
    LF: C,nv,nu,H,W or B,C,nv,nu,H,W
    ...
    if len(shape) == 5:
        ...
        show(make_grid(EPI_list,padding = 10,**kwargs))
    elif len(shape) == 6:
        B,C,nv,nu,H,W = shape
        EPI_list = [LF[:,:,yv_pair[1],:,yv_pair[0],:] for yv_pair in yv_pair_list]
...
def show_EPI_yv(LF,xu_pair_list,**kwargs):
    """
    LF: C,nv,nu,H,W or B,C,nv,nu,H,W
